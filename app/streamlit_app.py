...
def main():
    ...
    # Quản lý Session State để kết quả không bị mất khi bấm các nút tương tác
    if 'top_events' not in st.session_state:
        st.session_state['top_events'] = []
    if 'current_query' not in st.session_state:
        st.session_state['current_query'] = ""

    # Giao diện nhập liệu sử dụng st.form
    ...
    if submit_search and query:
        with st.spinner("Đang quét trên Qdrant và tính toán RRF..."):
            # Truyền tham số quét sâu xuống Backend
            ...
            # Chỉ dùng tham số hiển thị để slice mảng UI
    ...
    if st.session_state['top_events']:
        ...
        for idx, event in enumerate(top_events):
            ...
            with col:
                ...
                st.caption(f"🎥 `{video_id}` | ⏱️ `{start_time}s - {end_time}s`")
                
                if frame_path and os.path.exists(frame_path):
                    img = Image.open(frame_path)
                    st.image(img, use_container_width=True)
                elif match_type == "transcript_only":
                    # Tự động lấy thumbnail nhanh thay vì để hộp màu xanh
                    ...
                    if fast_img:
                        ...
                        st.info("Không thể tải video gốc.")
                
                # 2. Nút bấm Quét Mịn (Fine Search)
